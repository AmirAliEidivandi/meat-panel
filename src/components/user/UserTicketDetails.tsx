...
export default function UserTicketDetails() {
		...
		try {
			...
			if (response.message) {
				const newMessage: TicketMessageListItem = {
					...
					employee_id: response.message.employee_id ?? response.message.employee?.id ?? null,
					person_id: response.message.person_id ?? response.message.person?.id ?? null,
	...
	return (
		<div className='space-y-6 fade-in font-vazir'>
			...
			<div className='bg-white rounded-xl border-2 border-gray-200 overflow-hidden flex flex-col' style={{ height: '600px' }}>
				{/* Messages Header */}
														...
														>
															{isImage ? (
																...
																<a
																	...
																	target='_blank'
																	rel='noopener noreferrer'
																	className='flex items-center space-x-reverse space-x-2 text-sm text-emerald-600 hover:text-emerald-800'
																>
								...
								<textarea
									...
									onKeyDown={e => {
										if (e.key === 'Enter' && !e.shiftKey) {
											e.preventDefault();
											if (!replying && (replyMessage.trim() || uploadedFiles.length > 0)) {
												handleReply();
											}
										}
									}}
