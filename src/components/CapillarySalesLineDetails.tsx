...
export default function CapillarySalesLineDetails() {
  ...
  const getBranchText = (branch: string) => {
  ...
  const fetchSalesLineDetails = async () => {
    ...
    try {
      setLoading(true);
      setError("");
      const data = await capillarySalesLineService.getCapillarySalesLineById(
        salesLineId
      );
      ...
      setError(err?.message || "خطا در بارگذاری جزئیات خط فروش");
      setSalesLine(null);
    } finally {
      setLoading(false);
    }
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <p className="text-sm font-semibold text-gray-900">
            {getBranchText(salesLine?.branch?.name || "") || "ثبت نشده"}
          </p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex items-center space-x-reverse space-x-2 mb-4">
          ...
          <p className="text-sm font-semibold text-gray-900">
            {salesLine?.created_at ? formatDate(salesLine.created_at) : "N/A"}
          </p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex items-center space-x-reverse space-x-2 mb-4">
