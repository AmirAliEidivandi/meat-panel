...
import type { Customer, Employee, QueryWalletHistoryDto } from "../types";

interface WalletHistoryFilterModalProps {
  isOpen: boolean;
  onClose: () => void;
  onApplyFilters: (filters: QueryWalletHistoryDto) => void;
  currentFilters: QueryWalletHistoryDto;
}

const walletChangeTypes = [
  ...
  { value: "INITIAL_BALANCE_SET", label: "تنظیم موجودی اولیه" },
...
export default function WalletHistoryFilterModal({
  isOpen,
  onClose,
  onApplyFilters,
  currentFilters,
}: WalletHistoryFilterModalProps) {
  const [filters, setFilters] = useState<QueryWalletHistoryDto>(currentFilters);
  ...
  const handleFilterChange = (
    key: keyof QueryWalletHistoryDto,
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center space-x-reverse space-x-3">
            ...
            <h2 className="text-xl font-bold text-gray-900">
              فیلترهای پیشرفته تاریخچه کیف پول
            </h2>
          </div>
          <button
            onClick={onClose}
